  ...
  with HasCircularQueuePtrHelper
{
  ...
  dtlb.flatMap(a => a.ptw.req)
    ...
    val s1_hit = ptw_resp_next.data.s1.hit(tlb.bits.vpn, Mux(hasS2xlate, tlbcsr.vsatp.asid, tlbcsr.satp.asid), tlbcsr.hgatp.asid, allType = true, ignoreAsid = true, tlb.bits.s2xlate, ptw_resp_next.data.s2.entry.level.getOrElse(2.U))

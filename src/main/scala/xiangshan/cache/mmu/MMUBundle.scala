...
class PtwSectorResp(implicit p: Parameters) extends PtwBundle {
  ...
  def hit(vpn: UInt, asid: UInt, vmid: UInt, allType: Boolean = false, ignoreAsid: Boolean = false, s2xlate: UInt, s2_level: UInt): Bool = {
    ...
    val vmid_hit = Mux(s2xlate =/= noS2xlate, this.entry.vmid.getOrElse(0.U) === vmid, true.B)
    if (allType) {
      ...
      // printf(p"match vpn 0x${Hexadecimal(vpn)} <-> ptw tag 0x${Hexadecimal(entry.tag)}")
      // printf(p"hit0: ${hit0}, hit1: ${hit1}, hit2: ${hit2}, addr_low_hit: ${addr_low_hit}, vpn: 0x${Hexadecimal(vpn)}, tag: 0x${Hexadecimal(entry.tag)}, addr_low: 0x${Hexadecimal(addr_low)} \n")

      asid_hit && vmid_hit && Mux(entry.level.getOrElse(0.U) === 2.U || (s2xlate === allStage && s2_level === 2.U), hit2 && hit1 && hit0, Mux(entry.level.getOrElse(0.U) === 1.U || (s2xlate === allStage && s2_level === 1.U), hit1 && hit0, hit0)) && addr_low_hit
    } else {
...
class PtwRespS2(implicit p: Parameters) extends PtwBundle {
  ...
  def hit(vpn: UInt, asid: UInt, vasid: UInt, vmid: UInt, allType: Boolean = false, ignoreAsid: Boolean = false): Bool = { 
    val s1_hit = s1.hit(vpn, Mux(this.hasS2xlate(), vasid, asid), vmid, allType, ignoreAsid, this.s2xlate, this.s2.entry.level.getOrElse(2.U))

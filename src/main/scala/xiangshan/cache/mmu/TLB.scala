  ...
  with HasPerfEvents
{
  ...
  def handle_block(idx: Int): Unit = {
    ...
    val hit_s1 = io.ptw.resp.bits.s1.hit(miss_req_vpn, Mux(hasS2xlate, io.csr.vsatp.asid, io.csr.satp.asid), io.csr.hgatp.asid, allType = true, false, miss_req_s2xlate_reg, io.ptw.resp.bits.s2.entry.level.getOrElse(2.U))
  ...
  def ptw_resp_bypass(vpn: UInt, s2xlate: UInt) = {
    ...
    val normal_hit = ptw.resp.bits.s1.hit(vpn, Mux(hasS2xlate, io.csr.vsatp.asid, io.csr.satp.asid), io.csr.hgatp.asid, true, false, s2xlate, ptw.resp.bits.s2.entry.level.getOrElse(2.U))
